...
vocab_path = os.path.join(cur_dir, 'vocab_matNN.dict')
...
vocab_path2 = os.path.join(cur_dir, 'vocab_actN.dict')

# TODO !!! For consistency, this should be the ONLY place for loading ontology and vocab, and modifying them.
#      !!! Other files should import these data from here.
vocab1 = pickle.load(open(vocab_path2,'rb'))
...
for key in ontologyDict[u'informable']:
    ...
    ontologyDict[u'informable'][key].append('none')
...
# 在对句子中的slotval值进行tagging处理时，不同slot的处理顺序
...
def genTurnData_nbest(turn, labelJson):
    ...
    for asr_pair in user_input:
        ssum+= math.exp(float(asr_pair['score']))

    for asr_pair in user_input:
        asr_pair['score'] = math.exp(float(asr_pair['score']))/ssum
...
# def genTurnData_matFV(turn,labelJson):
#     tagged_turnData =genTurnData_nbest(turn, labelJson)
#     tag_dict = {}
#     for slot in ["food", "name"]:
#         val_ind = 1
#         for slot_val in ontology["informable"][slot]:
...
def main():
    ...
    res_data=gen_resdata(dataset,args.output_type)
    # write to json file
